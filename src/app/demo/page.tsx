...
export default async function DemoPage() {
  ...
  // Get project ID from cookie or use demo ID
  const cookieStore = await cookies();
  const projectId = cookieStore.get("project_id")?.value || "demo-project-123";
  
  // If authenticated and has a project, check onboarding state
  if (session?.user) {
    ...
    if (response.ok) {
      ...
      if (state.hasCompleteConfig && state.redirectTo) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-purple-800 to-indigo-900">
      ...
      <header className="p-6 bg-white/10 backdrop-blur-sm">
        <div className="container mx-auto flex items-center justify-between">
          ...
          <span className="text-white/60 text-sm">Demo Mode</span>
        </div>
      </header>
        ...
        <div className="max-w-4xl mx-auto space-y-8">
          ...
          <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-8 text-center">
            <h1 className="text-4xl font-bold text-white mb-4">
              Plum.ai Demo Dashboard
            </h1>
            <p className="text-purple-200 text-lg mb-8">
              Explore Plum's features with our demo data
            </p>
          </div>
          ...
          <div className="bg-yellow-500/20 backdrop-blur-sm rounded-xl p-6 text-center">
            <p className="text-yellow-200 text-sm">
              <strong>Note:</strong> This is a demo environment with sample data. 
              Actions taken here won't affect real Reddit posts.
            </p>
          </div>
        </div>
      </main>
