...
import { forwardRef, TextareaHTMLAttributes, InputHTMLAttributes, useEffect, useRef } from "react";
...
type GlassTextareaProps = BaseGlassInputProps &
  Omit<TextareaHTMLAttributes<HTMLTextAreaElement>, keyof HTMLMotionProps<"textarea">> & {
    type?: "textarea";
  };

type GlassInputFieldProps = BaseGlassInputProps &
  Omit<InputHTMLAttributes<HTMLInputElement>, keyof HTMLMotionProps<"input">> & {
...
export const GlassInput = forwardRef<
  ...
  GlassInputProps
>(({ variant = "ultra", shimmer = false, autoResize = true, className, type = "text", ...props }, ref) => {
  ...
  const focusStyles = {
    ...
    boxShadow: "inset 0 1px 3px rgba(0, 0, 0, 0.05), 0 0 0 1px rgba(168, 85, 247, 0.2), 0 0 20px rgba(168, 85, 247, 0.15)",
